document.addEventListener('DOMContentLoaded', function () {
  ...
  const WARNING_BEFORE   = 30 * 1000;      // show warning 30s before logout
  ...
  warningBanner.style.cssText = `
    ...
    text-align: center;
  `;
  warningBanner.innerHTML = '⚠️ Your session will close in <span id="countdown">30</span>s due to inactivity. <button onclick="resetInactivityTimer()" style="margin-left:10px;padding:4px 12px;border:none;border-radius:6px;background:#ffc107;color:#333;font-weight:700;cursor:pointer;">Stay logged in</button>';
  ...
  function startInactivityTimer() {
    ...
    warningTimer = setTimeout(() => {
      ...
      warningBanner._tick = tick;
    }, INACTIVITY_LIMIT - WARNING_BEFORE);

    inactivityTimer = setTimeout(() => {
      warningBanner.style.display = 'none';
      logout();
      // Show a brief message on the login screen
      const msg = document.createElement('div');
      msg.textContent = '🔒 Session closed due to inactivity.';
      msg.style.cssText = 'background:#fdecea;color:#c0392b;padding:12px 15px;border-radius:10px;margin-bottom:15px;border-left:4px solid #e74c3c;font-size:0.9em;';
  ...
  function stopInactivityTimer() {
    ...
    if (warningBanner._tick) clearInterval(warningBanner._tick);
    warningBanner.style.display = 'none';
  }
  ...
  function validateField(field) {
    if (field.id === 'docDate') return true; // always valid — has a default value
  ...
  function generatePDF() {
    ...
    const rawDate           = document.getElementById('docDate').value; // "YYYY-MM-DD"
    const date              = rawDate
    ...
    // ── SAVE ──────────────────────────────────────────────────────────────
    const fileName = `${docType}_${clientName.replace(/\s+/g, '_')}_${new Date().getTime()}.pdf`;
    doc.save(fileName);
